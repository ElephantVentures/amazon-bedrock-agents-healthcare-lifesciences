...
def search_pubmed(query: str, max_results: int = 100, max_records: int = None, rerank: str = "referenced_by") -> str:
    """
    Search PubMed for articles matching the query with optional citation analysis and ranking capabilities.
    ...
    within the result set to surface the most influential papers.

    Args:
        query: The search query for PubMed using standard PubMed search syntax
        max_results: Maximum number of results to fetch from initial search (default: 100, range: 1-1000)
        max_records: Maximum number of articles to return in final results (range: 1-100)
        rerank: Reranking method to apply (default: "referenced_by", options: ["referenced_by"])

    Returns:
        Formatted search results with article details, citation counts, and summary

    Examples:
        # Basic search with citation ranking
        result = search_pubmed("CRISPR gene editing", max_results=50, max_records=10)

        # Advanced search with temporal filters
        result = search_pubmed('mRNA vaccine COVID-19 AND "last 2 years"[dp]', max_results=200, max_records=20)
    """
    try:
        ...
        try:
            ...
            return f"Error building search query: {str(query_error)}"
        ...
        try:
            ...
            return f"HTTP error during PubMed search: {http_error.response.status_code} - {str(http_error)}"
        except httpx.TimeoutException as timeout_error:
            logger.error(f"Timeout error during PubMed search: {timeout_error}")
            return f"Request timeout during PubMed search: {str(timeout_error)}"
        except httpx.NetworkError as network_error:
            logger.error(f"Network error during PubMed search: {network_error}")
            return f"Network error during PubMed search: {str(network_error)}"
        except httpx.RequestError as request_error:
            logger.error(f"Request error during PubMed search: {request_error}")
            return f"Request error during PubMed search: {str(request_error)}"

        try:
            search_data = search_response.json()
        except Exception as json_error:
            logger.error(f"JSON parsing error in search response: {json_error}")
            return f"Error parsing search response: {str(json_error)}"
        ...
        try:
            ...
            return f"Unexpected search response format: missing {str(key_error)}"
        ...
        try:
            ...
            return f"Error fetching article details: {str(fetch_error)}"
        ...
        if rerank == "referenced_by":
            try:
                ...
                try:
                    ...
                    return f"Error formatting search results: {str(format_error)}"
        ...
        return f"Unexpected error during PubMed search: {str(unexpected_error)}"
